...
def log(message):
    ...
    print(log_format.format(time.ctime(), resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,
                            resource.getrusage(resource.RUSAGE_SELF).ru_utime, message))
...
def localtest():
    ...
    rslts, dur, nhash = matcher.match_file(audfprint_analyze.g2h_analyzer,
                                           hash_tab, qry)
    t_hop = 0.02322
    log_format = "Matched {} ({} s {} hashes) as {} at {} with {} of {} hashes"
    print(log_format.format(qry, dur, nhash, hash_tab.names[rslts[0][0]], t_hop*float(rslts[0][2]),
                            rslts[0][1], rslts[0][3]))
